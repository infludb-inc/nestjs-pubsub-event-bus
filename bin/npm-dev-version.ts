...
void (async (): Promise<void> => {
  ...
  const ticketNumber: string | undefined = /^(feature|feat|hotfix|fix)\/(\d+)/gim
    .exec(branchName)?.[0]
    ?.split('/')
    .pop();
  ...
  if (!semVer.prerelease.length && !ticketNumber) {
    throw new Error(
      `Your branch should contain the ticket number or the package.json version should be with the prerelease version`,
    );
  }
  if (
    semVer.prerelease.length &&
    Number.isSafeInteger(semVer.prerelease[semVer.prerelease.length - 1])
  ) {
    // @ts-ignore
