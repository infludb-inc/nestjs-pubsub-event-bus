...
export function PubsubEventHandler(
  ...params: Type<PubsubEventListener<any>>[] | EventsWithOptions
): ClassDecorator {
  if (!params.length) {
    return Reflect.metadata(PUBSUB_EVENT_HANDLER_METADATA, { events: [] });
  }
  const optionsOrEvent: IPubsubEventHandlerOptions | Type<PubsubEventListener<any>> =
