...
export class EventBus<EventBase extends IEvent = IEvent> extends NestEventBus {
  ...
  async registerPubsubEvents(handlers: EventHandlerType<EventBase>[] = []): Promise<void> {
    for (const handler of handlers) {
      const { events, autoAck = AutoAckEnum.ALWAYS_ACK }: IPubsubEventHandlerMetadata =
        this.reflectPubsubMetadata(handler);
      ...
      const eventNames: string[] = events.map((event: Type<PubsubEvent<any>>): string => {
        const routingKey: string =
          Reflect.getMetadata(PUBSUB_EVENT_NAME, event) ?? toEventName(event.name);
  ...
  protected registerPubsubHandler(
    ...
    if (!instance) {
      this.logger().warn(
        'Could not get event handler instance',
        JSON.stringify({ name: handler.name }),
      );
      return;
    }
  ...
  protected async bindPubsubConsumer(
    handler: EventHandlerType<EventBase>,
    events: string[],
  ): Promise<void> {
  ...
  protected reflectPubsubMetadata(
    handler: EventHandlerType<EventBase>,
  ): IPubsubEventHandlerMetadata {
    return Reflect.getMetadata(
      PUBSUB_EVENT_HANDLER_METADATA,
      handler,
    ) as IPubsubEventHandlerMetadata;
  }

  protected emitPubsubEvent = (
    handler: EventHandlerType<EventBase>,
    ...
    if (unused.length) {
      this.logger().warn(
        "Handler's event intersection detected",
        JSON.stringify({ ...context, unused }),
      );
    }

    const pubsubEvent: PubsubEvent<any> = new instance(
